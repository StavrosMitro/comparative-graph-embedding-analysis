"""
Hyperparameter tuning for classifiers on ENZYMES dataset.
...
# Cache directory for storing tuned parameters
...
def run_classifier_tuning(
    ...
    Args:
        ...
        fast_mode: Use reduced parameter grids for faster search
        cv: Number of cross-validation folds
        random_state: Random seed
        save_results: Whether to save results to cache
    
    Returns:
        Dictionary with tuned models and parameters for each classifier
    """
    print("\n" + "="*80)
    ...
    if save_results:
        ...
        cache_path = os.path.join(CACHE_DIR, 'enzymes_tuned_params.json')
        
        # Save only parameters (not models)
        params_to_save = {
            clf_name: {
                'params': res['params'],
...
def get_tuned_classifiers(tuned_params: Dict[str, Dict[str, Any]], random_state: int = 42) -> Dict[str, Any]:
    ...
    Args:
        tuned_params: Dictionary from load_tuned_params() or run_classifier_tuning()
        random_state: Random seed
    ...
    if 'SVM (RBF)' in tuned_params:
        ...
        # Extract SVC params (remove 'svc__' prefix)
...
def get_classifiers_with_tuning(
    X_train: np.ndarray,
    y_train: np.ndarray,
    use_cache: bool = True,
    fast_mode: bool = True,
    ...
    """
    Get classifiers, optionally with hyperparameter tuning.
    
    If cached tuned params exist and use_cache=True, use those.
    Otherwise, run tuning.
    
    Args:
        X_train: Training features (needed if tuning is required)
        y_train: Training labels (needed if tuning is required)
        use_cache: Whether to use cached parameters if available
        fast_mode: Use reduced parameter grids
        random_state: Random seed
    ...
    if tuned_params is None:
        ...
        tuned_params = {
            clf_name: {'params': res['params'], 'cv_score': res['cv_score']}
            for clf_name, res in tuning_results.items()
        }
    
    # Get tuned classifiers
    classifiers = get_tuned_classifiers(tuned_params, random_state)
    
    # Add MLP (not tuned - too slow)
    classifiers['MLP'] = make_pipeline(
        StandardScaler(),
        MLPClassifier(
