#!/usr/bin/env python3
# evaluate_saved_embeddings_merged.py
#
# Merged evaluator for SAVED embeddings.
#
# It combines functionality from:
#   1) evaluate_embeddings_all_models_cv_only_cheaty.py
#   2) g2v_enzymes_all_modes_missing_reqs.py (only the parts possible from saved embeddings)
#
# What this script does:
#   - Discovers saved embedding runs (expects labels.npy, graph_ids.npy, embeddings/*.npy|*.pt)
#   - For each embedding:
#       * FAST single-split hyperparameter tuning (cheaty, full data split)
#       * Final repeated stratified CV across full data (all models)
#       * Optional fixed SVC CV metrics (for stability deltas)
#       * Clustering (kmeans + optional spectral)
#   - Builds a dimension-sweep summary from discovered dimensions/epochs
#   - Computes Procrustes-aligned stability IF perturbed embeddings are present
#   - Produces t-SNE + optional UMAP plots for best clean embedding per run
#
# Notes:
#   - Training-time statistics from the original training script are not recoverable
#     unless you have sidecar metadata; this script evaluates saved embeddings only.
#   - Stability requires matching clean/perturbed embeddings:
#       same (dataset_root, run_name, D, epoch), with clean p=0 and perturbed p>0.
#     If no perturbation tags are found, stability section is skipped automatically.
...
# Roots containing run folders with:
#   run_dir/
#       graph_ids.npy
#       labels.npy
#       embeddings/*.npy|*.pt
DATASET_ROOTS: List[str] = ["./g2v_embeddings_selected_epochs_named"]

# [] => evaluate all discovered epochs
EPOCHS_TO_EVAL: List[int] = []

# ---------------------------------------------------------------------------
# FAST tuning (single split on FULL data; intentionally "cheaty")
# ---------------------------------------------------------------------------
FAST_TUNE_TRAIN_FRAC = 0.8
FAST_TUNE_SEED = 123
FAST_SELECT_METRIC = "f1_macro"  # "f1_macro" or "accuracy"
...
# ---------------------------------------------------------------------------
# Dimension sweep summary (from discovered embeddings)
# ---------------------------------------------------------------------------
BUILD_DIM_SWEEP_SUMMARY = True

# ---------------------------------------------------------------------------
# Stability from saved clean/perturbed embeddings
# - Requires p-tags in filename/path, e.g. "...p0.10..." or "...pert_0.10..."
# - Missing p-tag can be interpreted as clean (p=0) if enabled
# ---------------------------------------------------------------------------
COMPUTE_STABILITY = True
ASSUME_MISSING_P_IS_CLEAN = True
CLEAN_P_VALUE = 0.0

# Optional fixed model metrics for stability deltas (to mimic single-model protocol)
...
# ---------------------------------------------------------------------------
# Plots from saved embeddings
# - Plot best clean embedding per run (by best-model CV macro-F1)
# ---------------------------------------------------------------------------
MAKE_PLOTS = True
...
# Output
# ---------------------------------------------------------------------------
OUT_DIR = "./embedding_eval_results_merged"
SAVE_JSONL = True
SAVE_CSV = True
...
_P_INT_RE_LIST = [
    re.compile(r"(?:^|[^a-z0-9])p(?:ert)?[_\-]?([0-9]{1,2})(?:[^0-9]|$)", re.IGNORECASE),  # 5 -> 0.05, 10 -> 0.10
]
...
# =============================================================================
# FAST tuning split (single split on FULL data)
# =============================================================================
def stratified_train_val_split(
    ...
    try:
        ...
        print(f"[WARN] fast split failed ({e}); falling back to non-stratified split.")
        idx = np.arange(y.shape[0])
        tr, va = train_test_split(idx, train_size=train_frac, random_state=seed, stratify=None)
        return X[tr], X[va], y[tr], y[va]
...
def cluster_spectral(X: np.ndarray, y: np.ndarray, *, seed: int) -> Tuple[ClusterResult, np.ndarray]:
    if not HAS_SPECTRAL:
        raise RuntimeError("SpectralClustering not available in this sklearn install.")
    k = int(len(np.unique(y)))
...
def plot_2d_scatter(X2: np.ndarray, y: np.ndarray, title: str, out_path: str) -> None:
    ...
    plt.scatter(X2[:, 0], X2[:, 1], c=y, s=14)
    plt.title(title)
    plt.tight_layout()
...
def print_classification_summary(model_eval_records: List[ModelEvalRecord]) -> None:
    if not model_eval_records:
        return
    by_emb: Dict[Tuple[str, str, str], List[ModelEvalRecord]] = {}
    for r in model_eval_records:
        by_emb.setdefault((r.dataset_root, r.run_name, r.emb_path), []).append(r)

    print("\n" + "=" * 120)
    print("[SUMMARY] Classification: best CV macro-F1 model per embedding")
    print("=" * 120)
    for key in sorted(by_emb.keys()):
        recs = by_emb[key]
        best = sorted(recs, key=lambda rr: (-rr.cv_f1_mean, -rr.cv_acc_mean, rr.model_name))[0]
        print(
            f"- root={os.path.basename(best.dataset_root.rstrip('/')):<25s} run={best.run_name:<36s} "
            f"epoch={best.epoch:03d} D={best.D:<4d} p={fmt(best.p)} "
            f"best={best.model_name:<18s} "
            f"cv_acc={fmt(best.cv_acc_mean)}±{fmt(best.cv_acc_std)} "
            f"cv_f1={fmt(best.cv_f1_mean)}±{fmt(best.cv_f1_std)} "
            f"cv_auc={fmt(best.cv_auc_mean)}±{fmt(best.cv_auc_std)}"
        )

def print_clustering_summary(cluster_records: List[EmbeddingClusterRecord]) -> None:
    ...
    print("=" * 120)
    for r in sorted(cluster_records, key=lambda z: (z.dataset_root, z.run_name, z.epoch, z.D, z.emb_path)):
        print(
            f"- root={os.path.basename(r.dataset_root.rstrip('/')):<25s} run={r.run_name:<36s} "
            f"epoch={r.epoch:03d} D={r.D:<4d} p={fmt(r.p)} | "
            f"kmeans_ari={fmt(r.kmeans_ari)} kmeans_sil={fmt(r.kmeans_sil)} | "
            f"spectral_ari={fmt(r.spectral_ari)} spectral_sil={fmt(r.spectral_sil)}"
        )

def print_dim_sweep_summary(dim_rows: List[DimSweepSummaryRecord]) -> None:
    if not dim_rows:
        return
    print("\n" + "=" * 120)
    print("[SUMMARY] Dimension sweep (best clean embedding per D, per run)")
    print("=" * 120)
    for r in sorted(dim_rows, key=lambda x: (x.dataset_root, x.run_name, x.D)):
        print(
            f"- root={os.path.basename(r.dataset_root.rstrip('/')):<25s} run={r.run_name:<36s} D={r.D:<4d} "
            f"best_epoch={r.best_epoch:03d} best_model={r.best_model_name:<18s} "
            f"cv_acc={fmt(r.best_cv_acc_mean)} cv_f1={fmt(r.best_cv_f1_mean)} cv_auc={fmt(r.best_cv_auc_mean)}"
        )

def print_stability_summary(stab_rows: List[StabilityRecord]) -> None:
    if not stab_rows:
        print("\n[SUMMARY] Stability: no matched clean/perturbed pairs found.")
        return

    print("\n" + "=" * 120)
    print("[SUMMARY] Stability (Procrustes-aligned cosine + CV deltas)")
    print("=" * 120)
    for r in sorted(stab_rows, key=lambda x: (x.dataset_root, x.run_name, x.D, x.epoch, x.p)):
        print(
            f"- root={os.path.basename(r.dataset_root.rstrip('/')):<25s} run={r.run_name:<36s} "
            f"D={r.D:<4d} epoch={r.epoch:03d} p={r.p:.2f} "
            f"cos={fmt(r.mean_cosine_aligned)} "
            f"Δacc(best)={fmt(r.delta_acc_best_model)} Δf1(best)={fmt(r.delta_f1_best_model)} "
            f"Δacc(svc)={fmt(r.delta_acc_fixed_svc)} Δf1(svc)={fmt(r.delta_f1_fixed_svc)}"
        )
...
def main() -> None:
    ...
    # cache loaded embeddings for stability/plot
    emb_cache: Dict[str, np.ndarray] = {}

    print("=" * 120)
    print("[EVAL] Saved embeddings merged evaluator")
    print("=" * 120)
    ...
    print(f"cap combos/model      : {HYPERPARAM_MAX_COMBOS_PER_MODEL}")
    ...
    print(f"dim summary           : {BUILD_DIM_SWEEP_SUMMARY}")
    print(f"stability             : {COMPUTE_STABILITY} (assume missing p as clean={ASSUME_MISSING_P_IS_CLEAN})")
    print(f"plots                 : {MAKE_PLOTS} (best clean per run={PLOT_ONLY_BEST_CLEAN_PER_RUN})")
    print(f"outputs               : {OUT_DIR} overwrite={OVERWRITE_OUTPUT}")
    print("")
        ...
        for run_dir in runs:
            ...
            for emb_path in emb_files:
                ...
                Xtr, Xva, ytr, yva = stratified_train_val_split(
                    X, y,
                    train_frac=float(FAST_TUNE_TRAIN_FRAC),
                    seed=int(FAST_TUNE_SEED + epoch),
                )

                fast_results: List[FastTuneResult] = []
                print("    [FAST TUNE] selecting params per model (single split)")
                ...
                # Final CV all models
                print("    [CV] final repeated-stratified-CV on full data")
                ...
                # Fixed SVC metrics (optional) for stability deltas
                ...
                # keep cache for stability/plot (only if likely needed)
    ...
    # -------------------------------------------------------------------------
    # Stability summary (clean vs perturbed embeddings)
    # -------------------------------------------------------------------------
    ...
    if MAKE_PLOTS:
        # choose candidates
        candidates: List[EmbeddingSummaryRecord]
        clean = [
            r for r in emb_summary_records
            if r.p is not None and abs(float(r.p) - float(CLEAN_P_VALUE)) < 1e-12
        ]
        ...
        for r in sorted(candidates, key=lambda z: (z.dataset_root, z.run_name, z.epoch, z.D)):
            ...
            # load labels
            # find labels from run path by rediscovery assumption
            # faster: derive from r.emb_path -> run_dir
            ...
            try:
                ...
                title = f"{r.run_name} | epoch={r.epoch} D={r.D} | t-SNE"
                plot_2d_scatter(X2_tsne, ys, title, tsne_path)
            except Exception as e:
            ...
            if X2_umap is not None:
                try:
                    ...
                    title = f"{r.run_name} | epoch={r.epoch} D={r.D} | UMAP"
                    plot_2d_scatter(X2_umap, ys, title, umap_path)
                except Exception as e:
                    print(f"[WARN] UMAP plot save failed for {r.emb_file}: {e}")
    ...
    if SAVE_RESULTS_JSON:
        merged = {
            "config": {
                ...
                "assume_missing_p_is_clean": ASSUME_MISSING_P_IS_CLEAN,
    ...
    # Print summaries
    print_classification_summary(model_eval_records)
    print_clustering_summary(cluster_records)
    print_dim_sweep_summary(dim_rows)
    print_stability_summary(stab_rows)
