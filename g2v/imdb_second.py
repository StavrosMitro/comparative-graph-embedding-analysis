#!/usr/bin/env python3
# imdb_multi_evaluate_saved_embeddings_all_metrics.py
#
# Evaluates saved IMDB-MULTI embeddings produced by script 1.
#
# Includes:
#   - FAST tune (single train/val split on full data)
#   - Final repeated stratified CV for all models
#   - Clustering (kmeans + optional spectral)
#   - Dimension sweep summary (best clean embedding per D)
#   - Stability (Procrustes-aligned cosine + metric deltas) if perturbed embeddings exist
...
DATASET_ROOTS: List[str] = ["./g2v_imdb_multi_embeddings_selected_epochs_named/IMDB-MULTI"]
...
# Build per-D summary from clean embeddings
BUILD_DIM_SWEEP_SUMMARY = True

# Stability (requires embeddings with p-tags in file/path like "__p0.10__")
COMPUTE_STABILITY = True
ASSUME_MISSING_P_IS_CLEAN = True
CLEAN_P_VALUE = 0.0

# Fixed SVC CV metrics for stability deltas (optional)
...
OUT_DIR = "./imdb_multi_embedding_eval_results"
SAVE_JSONL = True
SAVE_CSV = True
...
_P_INT_RE_LIST = [
    re.compile(r"(?:^|[^a-z0-9])p(?:ert)?[_\-]?([0-9]{1,2})(?:[^0-9]|$)", re.IGNORECASE),  # 5 -> 0.05
]
...
def main() -> None:
    ...
    print("=" * 120)
    print("[EVAL] IMDB-MULTI saved embeddings")
    print("=" * 120)
    ...
    for dataset_root in DATASET_ROOTS:
        ...
        for run_dir in runs:
            ...
            for emb_path in emb_files:
                ...
                print("    [FAST TUNE] selecting params per model")
                ...
                # Final CV all models
                print("    [CV] all models on full data")
                ...
                # fixed svc for stability deltas
