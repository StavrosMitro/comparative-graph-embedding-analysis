...
# --- 2. CALCULATION CORE (UPDATED FOR FUNCTIONS) ---
def compute_spectral_distances_for_func(graphs, func_type='harmonic'):
    """
    Υπολογίζει τις φασματικές αποστάσεις ανάλογα με τη συνάρτηση f(λ).
    """
    ...
    for i, G in enumerate(graphs):
        ...
        try:
            ...
            # 2. Eigen-decomposition (Ιδιοτιμές w, Ιδιοδιανύσματα v)
            w, v = np.linalg.eigh(L)
            
            # 3. Εφαρμογή της συνάρτησης f(λ)
            if func_type == 'harmonic':
                ...
                # f(λ) = λ^2 (Local Structure)
                func_w = w ** 2
                
            elif func_type == 'biharmonic':
                # f(λ) = 1/λ^2 (Global Strong)
                with np.errstate(divide='ignore', invalid='ignore'):
...
def analyze_and_visualize(distances, node_counts, func_type):
    """
    Εκτυπώνει στατιστικά και σώζει το γράφημα με το όνομα της συνάρτησης.
    """
    save_path = f'analysis_{func_type}.png'
    ...
    sim_range = p99
    # Προσαρμόζουμε τα bins ανάλογα με το εύρος (αν είναι μικρό θέλουμε λιγότερα, αν μεγάλο περισσότερα)
